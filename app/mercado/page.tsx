...
import { Check, X, Loader2, ShoppingCart, ArrowLeft, Store } from "lucide-react";
...
export default function EstouNoMercadoView() {
  ...
  const toggleItem = async (item: any) => {
    ...
    if (item.comprado) {
      // REGRA: Tirar do carrinho apenas desmarcando
      const novosItens = sessao!.itens.map(i => i.id === item.id ? { ...i, comprado: false, preco: 0, qtd: 0 } : i);
      setSessao({ ...sessao!, itens: novosItens });
      await atualizarCompraAction(sessao!.nomeAba, item.id, { comprado: false, preco: 0, qtd: 0, mercadoNome: sessao!.mercadoNome });
    } else {
      setSelecionado(item);
      setForm({ preco: "", qtd: "1" });
    }
  ...
  const confirmarItem = async () => {
    ...
    const novosItens = sessao!.itens.map(i => i.id === selecionado.id ? { ...i, comprado: true, preco, qtd } : i);
    setSessao({ ...sessao!, itens: novosItens });
    await atualizarCompraAction(sessao!.nomeAba, selecionado.id, { comprado: true, preco, qtd, mercadoNome: sessao!.mercadoNome });
    setSelecionado(null);
  };
          ...
          <div className="bg-white w-full max-w-md rounded-t-[32px] sm:rounded-[32px] p-8 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div>
                ...
                <input type="number" value={form.preco} onChange={e => setForm({...form, preco: e.target.value})} className="w-full p-4 bg-slate-100 rounded-2xl text-xl font-bold" placeholder="0,00" autoFocus />
              </div>
              <div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmarItem} className="flex-[2] bg-green-600 text-white py-4 rounded-2xl font-black">SALVAR</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-6 z-[60]">
          <div className="bg-white rounded-[32px] p-8 w-full max-w-sm text-center shadow-2xl">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => setConfirmarFinalizacao(false)} className="w-full py-4 font-bold text-slate-400">VOLTAR</button>
            </div>
          </div>
        </div>
      )}
